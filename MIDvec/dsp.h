...
dvec_o conv(dvec_o xn, dvec_o hn) { //Convolve xn with hn. Output is same length as longest of xn and hn.
	int n = 0;
	int m = 0;
...
dvec_o corr(dvec_o xn, dvec_o hn) { //Correlate xn with hn. Output is same length as longest of xn and hn.
	int n = 0;
	int m = 0;
...
dvec_o conv_full_length(dvec_o xn, dvec_o hn) {
	...
	dvec_o rn = new_dvec(xn.len + hn.len - 1);
...
//FFT / IFFT.
cpx_vec_o fft(cpx_vec_o xn) { //Computes radix-2 Fast Fourier Transform of complex vector array.
...
dvec_o magfft(cpx_vec_o xn, int lengthchoose) { //Computes radix-2 Fast Fourier Transform of complex vector array.
...
cpx_vec_o ifft(cpx_vec_o Xk) { //Computes radix-2 Inverse Fast Fourier Transform of complex vector array.
	int n = 0;
...
dvec_o generate_lowpass_filter(double wL_1, double wL_2, double stopband_gain) { //Generate impulse response for windowed lowpass filter.
