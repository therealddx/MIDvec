...
cvec_o ifft(cvec_o Xk)
{
  ...
  cvec_o arg_xn;
  arg_xn.len = Xk.len;
  arg_xn.arr = (cpx_o*)malloc(sizeof(cpx_o)*arg_xn.len);
  
  cvec_o arg_xnbuf;
  arg_xnbuf.len = Xk.len;
  arg_xnbuf.arr = (cpx_o*)malloc(sizeof(cpx_o)*arg_xn.len);
  
  for (n = 0; n < arg_xn.len; n++) {
    arg_xn.arr[n].real = 0;
    arg_xn.arr[n].imag = 0;
    arg_xnbuf.arr[n].real = 0;
    arg_xnbuf.arr[n].imag = 0;
  }
  
  for (n = 0; n < (arg_xn.len >> 1); n++) {
    arg_xn.arr[n] = cpx_add(Xk.arr[n], Xk.arr[n + (Xk.len >> 1)]);
    arg_xn.arr[n + (arg_xn.len >> 1)] = cpx_sub(Xk.arr[n], Xk.arr[n + (Xk.len >> 1)]);
  }
  
  int depth = (int)(log10((double)arg_xn.len) / log10(2.0)) - 1;
  int y_int = 0;
  int gap = arg_xn.len >> 1;
  int inst_N = 4;
  int m = 0;
  ...
  while (depth > 0) {
    ...
    for (y_int = 0; y_int < (gap >> 1); y_int++) {
      for (m = 0; m < (inst_N >> 1); m++) {
        ...
        arg_xnbuf.arr[(gap >> 1) * m + y_int] = cpx_add(arg_xn.arr[gap * m + y_int], cpx_mul(tw, arg_xn.arr[gap * m + (gap >> 1) + y_int]));
        arg_xnbuf.arr[(gap >> 1) * m + y_int + (arg_xn.len >> 1)] = cpx_sub(arg_xn.arr[gap * m + y_int], cpx_mul(tw, arg_xn.arr[gap * m + (gap >> 1) + y_int]));
        
        //printf("Storing to: %d, %d\r\n", (gap>>1)*m+y_int, (gap>>1)*m+y_int+(arg_xn.len>>1));
        //printf("Reading from: %d, %d\r\n", gap*m+y_int, gap*m+(gap>>1)+y_int);
        //printf("%d / %d\r\n", tw_m, inst_N);
